document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (tableBody) {
      events.forEach((event: { id: number; nome_evento: string; lado_a: string; lado_b: string; data_evento: string }) => {
        ...
        row.innerHTML = `
          ...
          <td><button onclick="rejectEvent(${event.id}, '${userEmail}')">Reprovar</button></td>
        `;
